...
namespace TestProject.Console
{
	...
	public class ConsoleArgumentEngineTest
	{
		...
		[TestMethod]
		public void UsageSubSystemTestRequiredVariant()
		{
			var args = "--asd 123".Split(' ');
			var engine = new ConsoleArgumentEngine
			(
				new ConsoleArgumentEngine.PositionParameter()
				{
					Position = 0,
					Name = "posParam",
					IsRequired = true,
					ParseFunc = (s) => throw new InvalidOperationException("This method mustn't Invokes"),
				},
			...
			throw new Exception("Test Failed, engine.Calculate(string[]) has not throw exception");
		}

		[TestMethod]
		public void UsageSubSystemTestNonRequiredVariant()
		{
			var args = "--asd --a123".Split(' ');
			var engine = new ConsoleArgumentEngine
			(
				new ConsoleArgumentEngine.PositionParameter()
				{
			...
			var result = engine.Calculate(args);

			Assert.AreEqual(result["keyParam"], "--a123", $"result[\"keyParam\"]:{result["keyParam"]}");
			Assert.AreEqual(result["flag"], false, $"result[\"flag\"]:{result["flag"]}");
		}
	}
}
